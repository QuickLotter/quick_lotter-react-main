...
const POSITION_HEADERS = {
  "POSITION 06": [
    7, 1, 4, 2, 8, 10, 7, 6, 11, 5, 9, 15, 14, 12, 13, 16, 17, 20, 21, 18, 19,
...
const DATA_ROWS = Array.from({ length: 20 }, (_, i) => ({
  ...
  values: Array(POSITION_HEADERS["POSITION 06"].length)
    .fill(0)
    .map(() => Math.round(Math.random())),
}));
const FREQ = Array.from(
  { length: POSITION_HEADERS["POSITION 06"].length },
...
export default function Position1NYLotto() {
  ...
  const HEADER = POSITION_HEADERS["POSITION 06"];

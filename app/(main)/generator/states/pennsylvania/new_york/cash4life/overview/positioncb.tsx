...
// ====== HEADER PARA POSITION CB ======
// Você pode alterar essa ordem conforme a análise real da POSITION CB do Cash 4 Life
...
const DATA_ROWS = Array.from({ length: 20 }, (_, i) => ({
  ...
  values: Array(POSITION_HEADERS["POSITION CB"].length)
    .fill(0)
    .map(() => Math.round(Math.random())),
}));
const FREQ = Array.from(
  { length: POSITION_HEADERS["POSITION CB"].length },
  () => Math.floor(Math.random() * 350) + 10
);

export default function Position1Cash4Life() {
  ...
  const HEADER = POSITION_HEADERS["POSITION CB"];
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {/* HEADER PRINCIPAL */}
      <GameHeader
        logo={<Cash4LifeLogo width={100} height={40} />}
        title="Overview"
        subtitle="Cash 4 Life"
        headerColor="#2D7F67"
      ...
      <View style={styles.fixedHeader}>
        <View style={styles.filtersPad}>
          <View style={styles.datesRow}>
            <Text style={[styles.dateLabel, { color: "#3E4982" }]}>From</Text>
            <TouchableOpacity
              style={styles.input}
            ...
            <Text style={[styles.dateLabel, { color: "#3E4982" }]}>To</Text>
            <TouchableOpacity
              style={styles.input}
      ...
      <View style={styles.tableHeaderRow}>
        <View style={[styles.dateBoxHeader, { backgroundColor: "#3E4982" }]}>
        ...
        >
          <View style={{ flexDirection: "row" }}>
            {HEADER.map((n, i) => (
              <View
                key={i}
                style={[styles.headerNumberBox, styles.headerNumberBoxGreen]}
              >
                <Text
                  style={[
                    styles.headerNumberText,
                    styles.headerNumberTextGreen,
                  ]}
                >
                  {n}
                </Text>
      ...
      <ScrollView style={{ flex: 1 }}>
        {filteredRows.map((row, rowIndex) => (
          <View key={rowIndex} style={styles.gridRow}>
            <View style={[styles.dateBoxGrid, { backgroundColor: "#3E4982" }]}>

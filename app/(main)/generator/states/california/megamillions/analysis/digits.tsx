...
const VALUE_BOXES = [
  { label: "4", bgColor: "#1A1AFF", textColor: "#FFF" },
  { label: "5", bgColor: "#FF8C00", textColor: "#000" },
  { label: "6", bgColor: "#1B5E20", textColor: "#FFF" },
  { label: "7", bgColor: "#F8BBD0", textColor: "#000" },
  { label: "8", bgColor: "#FFD700", textColor: "#000" },
  { label: "9", bgColor: "#3E2723", textColor: "#FFF" },
];
const MOCK_ROWS = Array.from({ length: 30 }, (_, i) => ({
  date: `05/${(i + 1).toString().padStart(2, "0")}/25`,
  digits: 4 + (i % 6), // 4, 5, 6, 7, 8, 9
  values: Array(6)
    .fill(0)
    .map(() => 4 + Math.floor(Math.random() * 6)),
}));
const MOCK_FREQ = [95, 78, 50, 49, 29, 29];
...
export default function AnalysisDigits() {
  ...
  // useEffect(() => {
  //   fetchDigitsAnalysis(fromDate, toDate).then(({ rows, freq }) => {
  //     setRows(rows);
  //     setFreq(freq);
  //   });
  // }, [fromDate, toDate]);

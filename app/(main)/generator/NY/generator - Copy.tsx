...
export default function GeneratorNYUnified() {
  ...
  // ==== Lê selection de cada coluna da URL ====
  ...
  // Sempre cria 5 colunas, serve para Pick 3, 4, 5
  const pickColumns = [
    getColSelection("selected1"),
    getColSelection("selected2"),
    getColSelection("selected3"),
    getColSelection("selected4"),
    getColSelection("selected5"),
  ];

  // Normal games (Mega Millions etc)
  ...
  const [mainNumbers, setMainNumbers] = useState<number[]>(
    game.type === "normal" && initialSelected.length > 0 ? initialSelected : []
  );
  const [extraNumbers, setExtraNumbers] = useState<number[]>([]);

  // === PICK: monta cada coluna a partir dos parâmetros ===
  const [pickSelected, setPickSelected] = useState<number[][]>(() => {
    if (game.type === "pick") {
      return Array.from({ length: game.ballCount }, (_, i) =>
        pickColumns[i] ? pickColumns[i] : []
      );
    }
    return Array.from({ length: (game as any).ballCount || 4 }, () => []);
  });
  ...
  // Limpa tudo ao trocar de jogo ou URL
  useEffect(() => {
    setMainNumbers(
      game.type === "normal" && initialSelected.length > 0
        ? initialSelected
        : []
    );
    setExtraNumbers([]);
    if (game.type === "pick") {
      setPickSelected(
        Array.from({ length: game.ballCount }, (_, i) =>
          pickColumns[i] ? pickColumns[i] : []
        )
      );
    } else {

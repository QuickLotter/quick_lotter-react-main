...
export default function Position2Win4Midday() {
  ...
  const handleSave = () => {
    if (selectedNumbers.length > 0) {
      router.push(
        `/overview/ny/win4midday/position3?selected1=${selected1}&selected2=${selectedNumbers.join(
          ","
        )}`
      );
    }
          ...
          {TABS.map((tab, idx) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  router.replace(`/overview/ny/win4midday/${tab.route}`);

...
const TABS = [
  ...
  { label: "Position 06", route: "position6" },
];

// Cabeçalho das colunas
const POSITION_HEADERS = {
  "DRAWING SINCE": Array.from({ length: 59 }, (_, i) => i + 1),
};
...
const FREQ_59 = Array.from(
  { length: 59 },
  () => Math.floor(Math.random() * 350) + 10
);

export default function DrawingSinceNYLotto() {
          ...
          {TABS.map((tab, idx) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  router.replace(`/overview/new_york/nylotto/${tab.route}`);

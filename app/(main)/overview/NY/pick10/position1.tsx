...
const POSITION_HEADERS = {
  "POSITION 01": Array.from({ length: 80 }, (_, i) => i + 1),
};

// MOCK DATA (trocar pelo fetch do Supabase futuramente)
// (Opcional: ajuste o mock para simular 10 acertos por linha se quiser realismo)
const DATA_ROWS = Array.from({ length: 20 }, (_, i) => ({
  date: `05/${(i + 1).toString().padStart(2, "0")}/25`,
  values: Array(80)
    .fill(0)
    .map(() => Math.round(Math.random())), // 0 ou 1 aleatório
}));
...
export default function Position01Pick10() {

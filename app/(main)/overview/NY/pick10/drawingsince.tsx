...
const POSITION_HEADERS = {
  "DRAWING SINCE": Array.from({ length: 80 }, (_, i) => i + 1),
};
...
const FREQ_80 = Array.from(
  { length: 80 },
  () => Math.floor(Math.random() * 350) + 10
);

export default function DrawingSincePick10() {
  ...
  // >>>>>>>>>>>> CORES CUSTOMIZADAS <<<<<<<<<<<<<<<
  const headerColor = "#FFE363";
  const backArrowColor = "#000"; // <- cor da seta
  const titleColor = "#000"; // <- cor do título
  const subtitleColor = "#000"; // <- cor do subtítulo (opcional)
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.fixedHeader}>
        <View style={styles.filtersPad}>
          <View style={styles.datesRow}>
            <Text style={[styles.dateLabel, { color: "#000" }]}>From</Text>
            <TouchableOpacity
              style={styles.input}
            ...
            <Text style={[styles.dateLabel, { color: "#000" }]}>To</Text>
            <TouchableOpacity
              style={styles.input}
            ...
            <View style={[styles.input, { backgroundColor: "#F1F3F7" }]}>
              <Text
                style={[styles.inputText, { color: "#000", fontWeight: "700" }]}
              >
                {drawCount}
              </Text>
            </View>
      ...
      <View style={styles.tableHeaderRow}>
        <View style={[styles.dateBoxHeader, { backgroundColor: "#FFE363" }]}>
      ...
      <ScrollView style={{ flex: 1 }}>
        {filteredRows.map((row, rowIndex) => (
          <View key={rowIndex} style={styles.gridRow}>
            <View style={[styles.dateBoxGrid, { backgroundColor: "#FFE363" }]}>
